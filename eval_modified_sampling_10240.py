...
from  Custom_environment_persistent_surveillance_v5_v6_eval import UAV_UGV_Env
...
def get_best(sequences, cost, agents, ids=None, batch_size=None):
    ...
    return [sequences[i] if i >= 0 else None for i in result], [cost[i] if i >= 0 else math.inf for i in result], [agents[i] if i>=0 else None for i in result]
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    full_path = os.path.join(folder,'actions_RL_sampling_10240.csv')
...
def _eval_dataset(model, dataset, width, softmax_temp, opts, device):
    ...
    for batch_id, batch in enumerate (tqdm(dataloader, disable=opts.no_progress_bar)) :
        ...
        for seq, cost, agent in zip(sequences, costs, agents):
            ...
            elif model.problem.NAME == "mcsrp":
                seq = seq.tolist()
                agent = agent.tolist()
                
                action_with_agent = remove_consecutive_duplicates((list(zip(seq, agent))) )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--val_dataset', type=str, default='data/mcsrp/mcsrp20_test_seed1234.pkl', help='Dataset file to use for validation')
    parser.add_argument('--uav_graph_size', type=int, default=30, help="The size of the uav mission points graph")
    parser.add_argument('--ugv_graph_size', type=int, default=10, help="The size of the ugv mission points graph")
    ...
    parser.add_argument("--datasets", nargs='+', default=['data/mcsrp/mcsrp20_test_seed1234.pkl'], help="Filename of the dataset(s) to evaluate")
    ...
    parser.add_argument('--model', type=str, default = r'outputs\AM\epoch-99 (1).pt') #default = 'outputs/mcsrp_15/run_20231118T234822/epoch-63.pt',
...
#C:/Users/safwan/OneDrive - University of Illinois Chicago/Documents/PhD-ARL/RL/HADRL-f1b78d039f252bcefa154b0c5dd295f528e8dc89/Trial_4/outputs/mcsrp_15/run_20231121T161101/epoch-41.pt