...
def generate_uav_points(ugv_points, ua_p_d_f, safe_radius):
    ...
    if len(all_potential_points) > ua_p_d_f:
        # Calculate the weight for each point using a Gaussian PDF
        distances_x = all_potential_points[:, 0] - gaussian_center[0]
        distances_y = all_potential_points[:, 1] - gaussian_center[1]
        weights_x = stats.norm.pdf(distances_x, scale=sigma_x)
        weights_y = stats.norm.pdf(distances_y, scale=sigma_y)
...
def scenario_gen(uav_graph_size, ugv_graph_size):
