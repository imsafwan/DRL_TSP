...
''' 
Evaluation using greedy decoding and stochastic cases
...
from  Custom_environment_stochastic import UAV_UGV_Env
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    full_path = os.path.join(folder,'actions_RL_greedy.csv')
    ...
    for batch_id, batch in enumerate (tqdm(dataloader, disable=opts.no_progress_bar)) :
        ...
        with torch.no_grad():
            if opts.decode_strategy in ('sample', 'greedy'):
                ...
                sequences, costs = model.stochasticity_sample(batch, batch_rep=batch_rep, iter_rep=iter_rep)
                sequences, costs = model.sample_many(batch, batch_rep=batch_rep, iter_rep=iter_rep) ####### change is needed here  ####
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ugv_graph_size', type=int, default=5, help="The size of the uav mission points graph")
    ...
    parser.add_argument('--val_size', type=int, default=1*1,
    ...
    parser.add_argument('--model', type=str,  default = r'outputs\mcsrp_15\run_20240214T195016\epoch-69.pt')
